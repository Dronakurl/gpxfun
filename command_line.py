...
print(
    pd.pivot_table(ds, "dauer", index=["wochentag"], aggfunc=["count", "mean", np.std])
)
print(
    pd.pivot_table(ds, "dauer", index=["jahreszeit"], aggfunc=["count", "mean", np.std])
)
print(
    pd.pivot_table(ds, "dauer", index=["startzeit"], aggfunc=["count", "mean", np.std])
)
...
model = linear_model.ElasticNetCV(cv=10)
model.fit(X, y)
coeffs = pd.DataFrame([dummycols, pd.Series(lassomodel.coef_)]).T
print(coeffs)
print(lassomodel.intercept_)
