...
def infer_start_end(
    ...
    se_cluster["ende"] = se_cluster.apply(lambda x: Location(x["ende_lat"], x["ende_lon"], None), axis=1)
    if infer_by_distance:
        ...
        ds = ds[
            ds.apply(
                lambda x: (x["start"].distance_2d(x["start_x"]) < 150 and x["ende"].distance_2d(x["ende_x"]) < 150),
                axis=1,
            )
        ][["startendcluster", "startendcluster_x", "filename"]]
        d = d.merge(ds[["filename", "startendcluster_x"]], how="left").rename(
            {
        ...
        da = d[["startendcluster", "startendcluster_algo", "filename"]].copy()
        log.debug(
            pd.pivot_table(
                da,
                "filename",
    ...
    if log.level <= logging.DEBUG:
        da = d[["filename", "startendcluster"]].copy()
        print("infer_start_end: final cluster statistics")
        print(pd.pivot_table(da, "filename", index="startendcluster", aggfunc="count"))
